...
1. n-grams are then scored by the sum of the TFIDF of their consistuent tokens times the order of the n-gram (to prefer longer expressions),
$$\text{\ \ \ \ \ \ \ \ \ \ \ \ \ \ \ }n\ \sum_{i=1}^{|gram|} TFIDF(gram_i)$$ 
